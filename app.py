...
@app.route("/api/articles/<int:article_id>/summary", methods=["POST"])
@login_required
def api_article_summary(article_id):
    """Generate or return cached summary for an article using Claude API."""
    article = db.session.get(Article, article_id)
    if not article:
    ...
    if article.summary:
        return jsonify({"status": "ok", "summary": article.summary})

    # Fetch article content
    try:
        resp = http_requests.get(article.url, timeout=15, headers={
        ...
        # Truncate to ~4000 chars to keep API costs low
        if len(text) > 4000:
            text = text[:4000] + "..."
    except Exception as e:
        logger.warning("Failed to fetch article %s: %s", article.url, e)
        # Fall back to title-only summary
        text = article.title

    # Call Claude API for summary
    try:
        client = anthropic.Anthropic()
        is_korean = bool(re.search(r'[가-힣]', article.title))
        lang_instruction = "한국어로 작성해주세요." if is_korean else "Write in English."

        response = client.messages.create(
            model="claude-sonnet-4-20250514",
            max_tokens=500,
            messages=[{"role": "user", "content": (
                f"다음 기사를 간결하게 요약해주세요. {lang_instruction}\n\n"
                f"제목: {article.title}\n\n"
                f"본문:\n{text}\n\n"
                "형식:\n"
                "**핵심 요약**: (2-3문장으로 핵심 내용)\n"
        ...
        logger.info(
            "Article summary tokens: input=%d, output=%d",
            response.usage.input_tokens, response.usage.output_tokens,
        )
        return jsonify({"status": "ok", "summary": summary})
    except Exception as e:
        logger.error("Failed to generate summary for article %d: %s", article_id, e)
        return jsonify({"status": "error", "message": "요약 생성에 실패했습니다."}), 500
...
with app.app_context():
    ...
    with db.engine.connect() as conn:
        ...
        if "summary" not in columns:
            conn.execute(sqlalchemy.text("ALTER TABLE article ADD COLUMN summary TEXT"))
            conn.commit()
